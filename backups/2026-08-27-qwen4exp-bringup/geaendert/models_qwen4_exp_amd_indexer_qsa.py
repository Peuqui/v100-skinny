...
def apply_qsa_rope(
    ...
    if positions.ndim == 2:
        shape = tensor.shape
        # This fork's triton_mrope rotates the two halves of the rotary
        # dimension against each other, i.e. it implements NeoX style only and
        # takes no style argument. Qwen4Exp builds its rope with the get_rope
        # default (NeoX), so state the assumption rather than assume it.
        if not rotary_emb.is_neox_style:
            raise NotImplementedError(
                "QSA MRoPE requires NeoX-style rotation; this fork's "
                "triton_mrope implements no other layout."
            )
