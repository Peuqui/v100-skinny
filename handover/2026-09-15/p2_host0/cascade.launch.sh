...
export CUDA_VISIBLE_DEVICES=0,2,1,3,4
export VLLM_PP_LAYER_PARTITION=24,24
export VLLM_QWEN4EXP_PLE_HOST_GIB=0
export HOME=/home/mp
export VLLM_QWEN4EXP_PLE_STORE_DEVICE=4
export VLLM_QWEN4EXP_PLE_STORE_GIB=16
exec /home/mp/vllm/venv/bin/python -m vllm.entrypoints.openai.api_server --model /home/mp/.cache/huggingface/hub/models--nvidia--Qwen3.8-Flash-Next-NVFP4/snapshots/fc694b54fb0174e0913e6adf86691ef85a4ead47 --served-model-name Qwen3.8-Flash-Next-180B-A4B-NVFP4-MTP-vllm --trust-remote-code --dtype float16 --disable-custom-all-reduce --enable-auto-tool-choice --tool-call-parser qwen3_coder --reasoning-parser qwen3 --enable-prompt-tokens-details --enable-prefix-caching --distributed-timeout-seconds 3600 --tensor-parallel-size 2 --pipeline-parallel-size 2 --gpu-memory-utilization 0.95 --block-size 16 --max-model-len 262144 --max-num-seqs 4 --max-num-batched-tokens 2048 --host 127.0.0.1 --port 8093 --language-model-only --async-scheduling --speculative-config '{"method":"mtp","num_speculative_tokens":4,"draft_sample_method":"greedy"}' --compilation-config '{"cudagraph_capture_sizes":[1,2,4,5,8]}'
