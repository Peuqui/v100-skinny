...
class Qwen4ExpPLEFp8EmbeddingMethod(QuantizeMethodBase):
    ...
    def __init__(self, host_budget_bytes: int = 0) -> None:
...
def _ple_host_budget_bytes() -> int:
    """Host memory per rank the PLE table may use, from the environment.
    ...
    raw = os.getenv("VLLM_QWEN4EXP_PLE_HOST_GIB", "")
    if not raw.strip():
        return 0
    budget = float(raw)
    if budget < 0:
...
def _get_ple_embedding_quant_method(
    ...
    if not is_fp8(dtype):
        return None
    return Qwen4ExpPLEFp8EmbeddingMethod(_ple_host_budget_bytes())
