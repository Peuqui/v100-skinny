...
# Fork base names its AWQ config class differently than upstream 0.27.1
# (awq.AWQConfig vs auto_awq.AutoAWQConfig); same semantic check below.
...
from vllm.v1.attention.backends.gdn_attn_sm75 import GDNAttentionMetadata
...
@CustomOp.register("chunk_gated_delta_rule_sm75")
...
@PluggableLayer.register("qwen_gated_delta_net_attention_sm75")
class QwenGatedDeltaNetAttention(GatedDeltaNetAttention):
    def get_attn_backend(self):
        # sm75 pair: metadata for this layer must come from the upstream
        # backend copy, not the fork's registry entry for GDN_ATTN.
        from vllm.v1.attention.backends.gdn_attn_sm75 import GDNAttentionBackend
    ...
    def forward_hip(
        ...
        if GDN_AITER_TRITON_AVAILABLE:
            ...
            torch.ops.vllm.qwen_gdn_attention_core_sm75(
...
def qwen_gdn_attention_core_sm75(
...
direct_register_custom_op(
    op_name="qwen_gdn_attention_core_sm75",
    op_func=qwen_gdn_attention_core_sm75,
...
class QwenGatedDeltaNetAttentionForkCall(QwenGatedDeltaNetAttention):
    """Adapter for the fork's decoder-layer call convention.

    The fork's shared decoder layer (qwen3_next.py) calls
    ``linear_attn(hidden_states=..., output=<preallocated buffer>)`` and
    ignores the return value; upstream returns the result instead. The
    extra copy is the price for leaving the shared decoder code untouched.
    """
