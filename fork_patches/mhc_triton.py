...
def sm70_mhc_pre_norm_from_staging(
    ...
    if num_tokens == 1 and not hasattr(torch.ops._C, "sm70_glm_mhc_pre_norm_out"):
        # fork (v100-skinny): the 1.3.0 wheel predates the native final-stage
        # op. M=1 runs the same Triton final stage as M>1 (grid (1,), same
        # math) instead of upstream's hard requirement to rebuild from source.
        logger.info_once(
            "SM70 GLM mHC Triton decode final stage (wheel has no native op)."
        )
    elif num_tokens == 1:
