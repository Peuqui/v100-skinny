...
class Qwen4ExpPLEFp8EmbeddingMethod(QuantizeMethodBase):
    """FP8 PLE embedding, optionally split between device and host memory.

    The PLE table is by far the largest tensor of the checkpoint (51.2 GB, more
    than a third of the model) while contributing almost no arithmetic: a token
    reads 16 rows of 160 bytes. Keeping all of it in device memory is therefore
    the worst possible trade -- it displaces the KV cache, which is what limits
    the usable context.

    So the table is placed as a cascade: as many rows as fit stay in device
    memory, the remainder lives in pinned host memory and is read through a UVA
    view, which the gather reaches over PCIe without an explicit transfer. The
    rows are addressed by hashes and uniformly distributed, so the split point
    is a pure capacity decision and costs no accuracy.

    The two tables are allocated lazily, on the first checkpoint shard: by then
    the model is fully constructed and every other weight is allocated, which is
    exactly the information the placement needs. Allocating eagerly and moving
    rows out afterwards would need the full table plus its smaller copy alive at
    the same time -- 48.6 GB on a 48 GB card.
    """
    ...
    def __init__(
        ...
        # None means: derive the budget from the context requirement at
        # materialization time, when both terms are exactly known.
        self.host_budget_bytes = host_budget_bytes
        self.vllm_config = vllm_config
        self.total_rows = 0
        self.embedding_dim = 0
        self.placement: PLEPlacement | None = None
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        self.total_rows = sum(output_partition_sizes)
        self.embedding_dim = input_size_per_partition
        # Row count 0: the real tables are built by materialize_tables().
        weight = create_fp8_weight_parameter(
            0, input_size_per_partition, weight_loader
        )
    ...
    def _resolve_host_budget(self, device: torch.device) -> int:
        """Host bytes for the table: as configured, or derived from the context.

        Derived means: keep the table in device memory and spill only what the
        requested context needs beside it. Everything the calculation needs is
        exact at this point -- the model is fully built, every other weight of
        this stage is allocated, and each layer can state its KV requirement.
        """
        ...
        if self.vllm_config is None:
            raise RuntimeError(
                "PLE placement was asked to size itself but has no vLLM config; "
                "pass one, or set VLLM_QWEN4EXP_PLE_HOST_GIB to a fixed amount."
            )

        free_bytes, total_bytes = torch.cuda.mem_get_info(device)
        kv_bytes = kv_cache_bytes_for_max_model_len(self.vllm_config)
        reserve_bytes = _ple_vram_reserve_bytes(total_bytes)
        gmu = self.vllm_config.cache_config.gpu_memory_utilization
        budget = auto_ple_host_budget_bytes(
            table_bytes=self.total_rows * self.embedding_dim,
        ...
        logger.info(
            "PLE auto placement: %.2f GiB usable at gmu=%.2f, %.2f GiB already "
            "allocated, %.2f GiB needed for %d tokens of context, %.2f GiB held "
            "in reserve -> %.2f GiB of the table go to host memory",
    ...
    def _check_host_memory(self, placement: PLEPlacement) -> None:
        """Refuse a placement the host cannot hold, instead of dying in the OOM killer.

        Pinned memory cannot be swapped, and with tensor parallelism every rank
        pins its own share at the same time, so the reading here is optimistic
        by design -- it catches the clear-cut case, not the race.
        """
        ...
        if available is not None and needed > available:
            raise RuntimeError(
                f"PLE placement needs {needed / 1024**3:.2f} GiB of pinned host "
                f"memory but only {available / 1024**3:.2f} GiB is available, and "
                f"every tensor-parallel rank pins its own share. Lower the "
                f"requested context, or move layers to another pipeline stage."
            )

    def materialize_tables(self, layer: nn.Module) -> PLEPlacement:
        """Allocate the device and host halves of the table."""
        ...
        self._check_host_memory(placement)
        layer.weight.data = torch.empty(
            (placement.vram_rows, self.embedding_dim), dtype=dtype, device=device
        )

        if placement.host_rows:
            if not should_pin_memory():
                raise RuntimeError(
                    "PLE host placement needs pinned memory, which this platform "
                    "reports as unavailable; the gather reads it through a UVA "
                    "view. Unset the host budget to keep the table on the device."
                )
            host_storage = torch.empty(
                (placement.host_rows, self.embedding_dim), dtype=dtype, device="cpu"
            ).pin_memory()
        else:
            host_storage = torch.empty(
                (0, self.embedding_dim), dtype=dtype, device="cpu"
            )
        # The load path writes into the host tensor, the gather reads the UVA
        # view; both address the same memory.
        layer.ple_host_storage = host_storage
        layer.ple_host_table = (
            get_accelerator_view_from_cpu_tensor(host_storage)
            if placement.host_rows
            else host_storage.to(device)
        )

        self.placement = placement
        logger.info(
            "PLE table placement: %d of %d rows on device (%.2f GiB), "
            "%d rows in host memory (%.2f GiB)",
            placement.vram_rows,
            placement.total_rows,
            placement.vram_rows * self.embedding_dim / 1024**3,
            placement.host_rows,
            placement.host_rows * self.embedding_dim / 1024**3,
        )
        return placement
    ...
    def load_shard(
        ...
        *,
        ...
        self.materialize_tables(layer)
        return copy_ple_embedding_shard_split_(
            layer.weight.data,
            layer.ple_host_storage,
            loaded_weight,
    ...
    def process_weights_after_loading(self, layer: nn.Module) -> None:
        if self.placement is None:
            raise RuntimeError(
                "PLE table was never materialized -- no embedding shard reached "
                "load_shard(). The checkpoint does not carry the expected "
                "ngram_embedding.shard_<n>.weight entries."
            )
    ...
    def embedding(self, layer: nn.Module, input_: torch.Tensor) -> torch.Tensor:
        return split_ple_embedding_lookup(
            input_, layer.weight, layer.ple_host_table
        )
...
def _ple_vram_reserve_bytes(device_total_bytes: int) -> int:
    ...
    the engine refuse the requested context with a clear message.
    """

    raw = os.getenv("VLLM_QWEN4EXP_PLE_VRAM_RESERVE", "").strip()
    fraction = float(raw) if raw else 0.06
    if not 0.0 <= fraction < 1.0:
        raise ValueError(
            f"VLLM_QWEN4EXP_PLE_VRAM_RESERVE must be in [0, 1), got {fraction}"
        )
    return int(device_total_bytes * fraction)


def _get_ple_embedding_quant_method(
    config: Qwen4ExpTextConfig,
) -> QuantizeMethodBase | None:
    """Keep the PLE table in the storage dtype the checkpoint declares.

    The checkpoint states the table's dtype in ``ple_embedding_dtype``. That
    declaration is the only reliable signal here: the PLE rows are excluded
    from the NVFP4 quantization config (``ignore: ["*.ple.*"]``) yet stored as
    FP8, so no quantization config describes them. Expanding the table to the
    model dtype instead would double a 51 GB tensor, which no single card of
    this deployment can hold.
    """

    declared = getattr(config, "ple_embedding_dtype", None)
    if declared is None:
        return None
    dtype = getattr(torch, str(declared).removeprefix("torch."), None)
    if not isinstance(dtype, torch.dtype):
        raise ValueError(
            f"ple_embedding_dtype={declared!r} does not name a torch dtype"
        )
    if not is_fp8(dtype):
        return None
    return Qwen4ExpPLEFp8EmbeddingMethod(
        _ple_host_budget_bytes(), get_current_vllm_config()
    )
...
class Qwen4ExpNGramEmbedding(nn.Module):
    def __init__(
        ...
        self.ngram_embedding = VocabParallelEmbedding(
            ...
            quant_method=_get_ple_embedding_quant_method(config),
        )
        self.register_buffer(
            "positions_buffer",
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, loaded_weight in weights:
            ...
            if name.startswith(shard_prefix) and name.endswith(".weight"):
                ...
                method = getattr(embedding, "quant_method", None)
                if isinstance(method, Qwen4ExpPLEFp8EmbeddingMethod):
                    # The table may be split between device and host memory;
                    # the method owns that layout and the lazy allocation.
                    method.load_shard(
                        embedding,
                        loaded_weight,
                        checkpoint_start=checkpoint_start,
...
def qwen4_exp_amd_ple_ngram_embedding(
    ...
    # Dequantize here rather than in the caller: the FP8 tensor must not reach
    # Inductor, whose Triton backend has no fp8e4nv on sm70/sm75.
    output.copy_(layer._dequantize_embeddings(result, output.dtype))
