...
wait_free; echo "== E-1 RTX (TRITON_ATTN)"
env VLLM_ATTENTION_BACKEND=TRITON_ATTN VENV=$V DEVS=0,2 VLLM_SM70_QUANT_BACKEND=auto bash tools/mtp-diagnostics/speed_dflash.sh e1_tri_rtx upstream mtp > $HOME/.cache/mtp-diagnostics/sweep_e1_tri_rtx.log 2>&1; summ e1_tri_rtx
wait_free; echo "== V100 Referenz (TurboMind, TRITON_ATTN)"
env VLLM_ATTENTION_BACKEND=TRITON_ATTN VENV=$V DEVS=1,3 VLLM_SM70_QUANT_BACKEND=auto bash tools/mtp-diagnostics/speed_dflash.sh e1_tri_v100 upstream mtp > $HOME/.cache/mtp-diagnostics/sweep_e1_tri_v100.log 2>&1; summ e1_tri_v100
echo E1-ENDE
