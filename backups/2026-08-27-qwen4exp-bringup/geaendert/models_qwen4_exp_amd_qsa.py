...
class Qwen4ExpQSAFlashAttentionImpl(FlashAttentionImpl):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        if self.kv_cache_dtype not in ("auto", "bfloat16", "fp16"):
            raise NotImplementedError(
                "Qwen4Exp QSA requires an unquantized half-precision main KV cache"
            )
    ...
    def forward_qsa(
        ...
        # The kernel wants [blocks, block_size, kv_heads, head_size] (it reads
        # kv_heads off dim 2). Upstream stores K and V concatenated in the last
        # dimension of a head-major cache and transposes; this fork already
        # keeps that order and puts K/V on their own axis, so splitting that
        # axis is all it takes.
        ...
        if key_cache.dtype != query.dtype or query.dtype not in (
            ...
            raise NotImplementedError(
                "Qwen4Exp QSA requires Q/K/V in one common half-precision dtype"
            )
    ...
    def __init__(
        ...
        *,
        ...
        if model_config.dtype not in (torch.bfloat16, torch.float16):
            raise NotImplementedError("Qwen4Exp QSA requires a half-precision model")
        ...
        # Qwen3NextAttention._project_qkv_gate, which this owner reuses, keys
        # its per-layer tensor dumps on these two attributes.
        self.layer_idx = int(layer_id)
        ...
        if self.kv_cache_torch_dtype not in (torch.bfloat16, torch.float16):
            raise NotImplementedError(
                "Qwen4Exp QSA requires half-precision cache storage"
            )
