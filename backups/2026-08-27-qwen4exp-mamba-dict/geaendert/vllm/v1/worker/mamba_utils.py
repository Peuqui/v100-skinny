...
def assert_uniform_cache_scheduling(
    mamba_groups: dict[MambaSpec, list[int]],
) -> MambaSpec:
    """Return the reference spec after checking shared scheduling parameters."""
...
def count_mamba_states(
    kv_cache_config: KVCacheConfig,
    mamba_group_ids: list[int],
    copy_funcs: MambaStateCopyFuncsByType,
) -> int:
    """Count the physical state tensors across all mamba groups.

    Different groups may expose a different number of state tensors (GDN has
    two, the PLE short-conv one), so the count cannot be derived from a single
    ``num_state_types``.
    """
    return sum(
        len(
...
class MambaCopyBuffers:
    ...
    def create(
        ...
        mamba_spec = assert_uniform_cache_scheduling(mamba_groups)
        mamba_group_ids = get_mamba_group_ids(mamba_groups)
        entries_per_req = count_mamba_states(
            kv_cache_config, mamba_group_ids, copy_funcs
        )
...
class MambaSpecDecodeGPUContext:
    ...
    # different numbers of state types (for example, GDN has two while the PLE
    # short-conv has one).
    ...
    def create(
        ...
        mamba_spec = assert_uniform_cache_scheduling(mamba_groups)
        mamba_group_ids = get_mamba_group_ids(mamba_groups)
        total_states = count_mamba_states(
            kv_cache_config, mamba_group_ids, copy_funcs
        )
    ...
    def initialize_from_forward_context(
        ...
        Args:
            ...
            mamba_state_copy_funcs: Mapping from MambaAttentionBackendEnum to
                the copy functions of that mamba type, used to determine whether
                each state is a conv or temporal state.
        ...
        assert idx == self.num_states, (
            f"populated {idx} mamba states, but allocated {self.num_states}"
        )
