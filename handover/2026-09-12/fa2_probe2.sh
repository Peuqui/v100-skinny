...
CURL = "http://127.0.0.1:8066/v1/chat/completions"
def chat(prompt, mt):
    ...
    t0 = time.perf_counter()
    with urllib.request.urlopen(urllib.request.Request(CURL, data=b, headers={"Content-Type": "application/json"}), timeout=1800) as r:
        d = json.load(r)
    return time.perf_counter() - t0, d
tt = []
for i in range(3):
    t, d = chat(ctx, 1); tt.append(t)
tt.sort(); res["ctx_prompt_tokens"] = d["usage"]["prompt_tokens"]; res["ttft13k_s_median"] = round(tt[1], 2)
dr = []; toks = []
for i in range(3):
    t, d = chat(ctx, 200); n = d["usage"]["completion_tokens"]; toks.append(n); dr.append(n / (t - tt[1]))
dr.sort(); res["decode13k_tok_s_median"] = round(dr[1], 2); res["ctx_completion_tokens"] = toks
res["ctx_finish"] = d["choices"][0]["finish_reason"]; txt13 = d["choices"][0]["message"]["content"] or ""
res["ctx_sha"] = hashlib.sha256(txt13.encode()).hexdigest()[:16]; res["ctx_text_head"] = txt13[:160]
open(W + "/ctx_response.json", "w").write(json.dumps(d, indent=2))
