...
def _detect_torch_cuda_arch() -> str | None:
    ...
    if not torch.cuda.is_available():
        return None
    # Compile for the process's active device, not device 0: in mixed-GPU
    # setups (e.g. Turing + Volta pipeline stages) device 0 may be a
    # different architecture than the one this process runs kernels on.
    device = torch.cuda.current_device() if torch.cuda.is_initialized() else 0
