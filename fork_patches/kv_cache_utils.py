...
def _get_kv_cache_config_csa_linear(
    ...
    """Plan the CSA-linear cache as aliased tensors, or None if not CSA-linear.

    Upstream writes this layout as byte offsets into one backing store. This
    fork's allocator hands every layer of a KVCacheTensor the same buffer from
    byte 0, so "two owners at the same offset" is spelled as one shared
    tensor. The partition and the total size are the same either way -- one
    block holds, per QSA layer, a main_kv page and a compressed page:

        main_kv[i] + mamba[*][i]   |   compressed[i] + compressor_state[i]

    Each column above is one KVCacheTensor. The compressor-state and mamba
    specs were padded to their partner's page size when the groups were built
    (see _get_kv_cache_groups_csa_linear), so every alias set agrees on its
    page size.
    """
    ...
    num_blocks = available_memory // csa_layout.bytes_per_block
    num_blocks = may_override_num_blocks(vllm_config, num_blocks)

    # main_kv slot i carries the i-th layer of every mamba group.
    main_kv_aliases: list[list[str]] = [[name] for name in csa_layout.main_kv_names]
    for group in csa_layout.mamba_groups:
        if len(group.layer_names) > len(main_kv_aliases):
            raise ValueError(
                f"CSA+linear mamba group has {len(group.layer_names)} layers "
                f"but only {len(main_kv_aliases)} main_kv tensor slots exist; "
                "realign the pipeline partition."
            )
        for index, name in enumerate(group.layer_names):
            main_kv_aliases[index].append(name)

    kv_cache_tensors = [
        KVCacheTensor(
            size=csa_layout.main_kv_page_size * num_blocks,
            shared_by=shared_by,
        )
        for shared_by in main_kv_aliases
    ]
    kv_cache_tensors.extend(
        KVCacheTensor(
            size=csa_layout.compressed_page_size * num_blocks,
            shared_by=[compressed_name, state_name],
        )
        for compressed_name, state_name in zip(
...
def _get_kv_cache_groups_csa_linear(
    ...
    prefix_match_unit = vllm_config.cache_config.hash_block_size
    if (
