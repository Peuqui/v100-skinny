...
class Qwen4ExpMultiTokenPredictor(nn.Module):
    ...
    def forward(
        ...
        # The MTP drafter lives entirely on the last pipeline stage and is
        # single-stage from its own point of view, so branch on the input it
        # actually got rather than on the global PP rank. Same fix as
        # qwen3_5_mtp.py.
...
class Qwen4ExpMTP(nn.Module, SupportsPP, Qwen4ExpMixtureOfExperts):
    ...
    def get_top_tokens(
        ...
        # Required by this fork's local-argmax reduction for draft models;
        # same one-liner as qwen3_5_mtp.py.
