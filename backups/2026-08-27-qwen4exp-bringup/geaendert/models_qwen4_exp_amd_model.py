...
from vllm.model_executor.layers.mamba.gdn.qwen_gdn_linear_attn import (
    QwenGatedDeltaNetAttentionUpstreamCall as QwenGatedDeltaNetAttention,
)
...
# The checkpoint stores every fused projection as its separate parts, while the
# runtime packs them into one parameter. AutoWeightsLoader has no per-model
# stacked_params_mapping, so the fusion is declared here and carried to the
# weight loader as a shard id. Trailing dots keep the patterns disjoint: none
# of them matches a name another entry already rewrote.
_STACKED_WEIGHTS_MAPPER = WeightsMapper(
    orig_to_new_stacked={
        ...
        "mlp.shared_expert.gate_proj.": ("mlp.shared_expert.gate_up_proj.", 0),
        "mlp.shared_expert.up_proj.": ("mlp.shared_expert.gate_up_proj.", 1),
    }
)
...
class Qwen4ExpSparseMoeBlock(Qwen3NextSparseMoeBlock):
    ...
    def __init__(self, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        # Qwen4Exp loads its experts through AutoWeightsLoader, which descends
        # into FusedMoE.load_weights and needs the checkpoint mapping on the
        # layer. The counts are the ones the layer above was constructed with,
        # so the mapping always describes the experts that actually exist.
        ...
        if layer_type == "linear_attention":
            if (
                ...
                from vllm.model_executor.layers.mamba.gdn.qwen_gdn_linear_attn_sm75 import (  # noqa: E501
                    QwenGatedDeltaNetAttention as QwenGatedDeltaNetAttentionSM75,
                )
...
class Qwen4ExpModel(nn.Module):
    hf_to_vllm_mapper = getattr(Qwen3_5Model, "hf_to_vllm_mapper", WeightsMapper()) | _STACKED_WEIGHTS_MAPPER | _HC_WEIGHTS_MAPPER
    ...
    def forward(
        ...
        assert not isinstance(final_mixer, PPMissingLayer)
...
class Qwen4ExpForCausalLM(
    ...
    def get_top_tokens(self, hidden_states: torch.Tensor) -> torch.Tensor:
        # This fork's greedy fast path calls these on the inner model; the
        # outer wrapper only forwards. Same one-liners as qwen3_5.py.
