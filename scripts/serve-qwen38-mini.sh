...
# Mini-Anpassung: nur die GPUs prüfen, die dieser Boot wirklich nutzt —
# auf der Kiste läuft produktiv ein VLM-Side-Channel auf einer anderen Karte.
USED=$(nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits ${CUDA_VISIBLE_DEVICES:+-i "$CUDA_VISIBLE_DEVICES"} | sort -n | tail -1)
...
setsid $NUMA_PREFIX "$PY" -m vllm.entrypoints.openai.api_server \
  ...
  ${ASYNC_SCHED:+--async-scheduling "$ASYNC_SCHED"} \
...
if [ "$K" -gt 0 ]; then
  ...
  grep -q "num_speculative_tokens" "$LOG" \
    && gate "speculation off (K=0)" fail "speculative config present" \
    || gate "speculation off (K=0)" ok
fi
...
if [ "${EXPECT_FP8:-1}" = 0 ]; then
  gate "FP16 KV (checkpoint declares no KV directive)" ok
else
...
if [ "${EXPECT_FP8:-1}" = 0 ]; then CENSUS_EXPECTED=0; else CENSUS_EXPECTED=$((128 * TP)); fi
...
if [ "${EXPECT_FP8:-1}" = 0 ]; then
  gate "QPN8 skipped (all-NVFP4 checkpoint)" ok
else
