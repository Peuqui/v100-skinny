...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    def sample_tokens(
        ...
        if not self.is_last_pp_rank:
            ...
            # Fork fix (v100-skinny): take this step's draft token ids from the
            # last stage. Predicate must be num_speculative_steps, NOT
            # `self.speculator is not None` -- the speculator is built on the
            # last rank only, so mirroring that test would skip the receive and
            # deadlock the collective.
            if self.num_speculative_steps:
                drafts = pp_receive_drafts(
                    input_batch.num_reqs, self.num_speculative_steps
                )
                self.req_states.draft_tokens[input_batch.idx_mapping] = drafts
        ...
        if self.use_pp:
            ...
            # Fork fix (v100-skinny): the wire shape must match what
            # pp_receive allocates above, or the broadcast deadlocks.
            assert (
        ...
        if self.speculator is not None:
            ...
            # Fork fix (v100-skinny): mirror of pp_receive_drafts above. Same
            # predicate on both sides so the pair either fires or stays silent
            # together.
            if self.use_pp and self.num_speculative_steps:
                pp_broadcast_drafts(draft_tokens, input_batch.num_reqs)
