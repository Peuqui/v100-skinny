...
export VLLM_SM70_INDEXER_PREFILL_TILE_MB=64
...
export HOME=/home/mp
exec /home/mp/vllm/venv/bin/python -m vllm.entrypoints.openai.api_server --model /home/mp/models/DeepSeek-V4-Flash-nvfp4-DSpark --served-model-name DeepSeek-V4-Flash-nvfp4-DSpark-vllm --trust-remote-code --dtype half --kv-cache-dtype fp8 --disable-custom-all-reduce --enable-auto-tool-choice --tool-call-parser deepseek_v4 --reasoning-parser deepseek_v4 --enable-prompt-tokens-details --enable-prefix-caching --distributed-timeout-seconds 3600 --tensor-parallel-size 1 --pipeline-parallel-size 5 --gpu-memory-utilization 0.95 --max-model-len 65536 --max-num-seqs 1 --max-num-batched-tokens 64 --num-gpu-blocks-override 600 --host 127.0.0.1 --port 8093 --speculative-config '{"method":"dspark","num_speculative_tokens":5}' --compilation-config '{"cudagraph_capture_sizes":[6],"max_cudagraph_capture_size":6}'
