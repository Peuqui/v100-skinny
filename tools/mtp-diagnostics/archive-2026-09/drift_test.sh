...
run_phase () {  # $1=label  $2=QPN  $3=wipe(0/1)
  ...
  export VLLM_SM70_QUANT_BACKEND=auto VLLM_SKINNY_NVFP4=1 VLLM_SKINNY_QPN2=1
  ...
  curl -s -m 300 "http://127.0.0.1:$PORT/v1/completions" -H "Content-Type: application/json" \
    -d '{"model":"drift","prompt":"Nenne die ersten zehn Primzahlen und erklaere kurz, warum 1 keine Primzahl ist.","temperature":0,"max_tokens":80,"seed":7}' \
    > "$SP/drift_$label.json" 2>&1
...
echo "=== Phase 1 (QPN=0, leerer Cache)"; run_phase B_clean 0 1
echo "=== Phase 2 (QPN=1, leerer Cache)"; run_phase A_clean 1 1
echo "=== Phase 3 (QPN=0, Cache von A)";  run_phase B_cached 0 0
echo "FERTIG-TEIL1"
