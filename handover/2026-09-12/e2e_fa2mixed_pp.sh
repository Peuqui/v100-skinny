...
case $ARM in
  ...
  # mixedpp: 27B als TP1 PP2, Stufe 0 auf der RTX (PCI 0), Stufe 1 auf der V100 (PCI 1), OHNE Spekulation:
  #       der MTP-Entwurfskopf hat auf main kein SupportsPP (speculative.py verify_with_parallel_config),
  #       der 27B selbst erbt es ueber die VL-Huelle. Beide FA2-Bibliotheken in einem Modell.
  mixedpp)   ENTRY=Qwen3.8-27B-NVFP4-vllm; DEVS=0,1; ARCH="7.0;7.5"; SPEC_STRIP=0; EXTRA="--kv-cache-dtype float16"; SUBST="v100mem pp2 nospec" ;;
...
if extra:
    cmd += " " + extra
import re
SUBST = {"nospec": [("SPECRE", "")], "v100mem": [("--max-model-len 262144", "--max-model-len 32768"), ("--gpu-memory-utilization 0.98", "--gpu-memory-utilization 0.90")],
...
for key in subst.split():
    for a, b in SUBST[key]:
        if a == "SPECRE":
            assert re.search(r" --speculative-config '[^']*'", cmd), "keine Spekulationskonfiguration im Befehl"
            cmd = re.sub(r" --speculative-config '[^']*'", "", cmd)
            continue
