...
| `mhc_tilelang.py` | `vllm/model_executor/kernels/mhc/tilelang.py` | Upstream-HEAD dispatch (fp16 runs the dtype-parameterized TileLang kernels; on SM70 with the DSV4 shape `(4, 4096)` the Triton decode route from `mhc_triton.py` takes over, M 1–8). Fork additions: the torch reference implementations and `mhc_post_fp32` — the DSpark aux-hidden-state extraction reads the post reconstruction without the final fp16 cast (BOS attention-sink overflow). The historic fork-wide fp16→torch bypass is gone; it predated the dtype-parameterized kernels and cost ~0.8 ms/layer. |
| `mhc_triton.py` | `vllm/model_executor/kernels/mhc/triton.py` | Upstream-HEAD backport: the SM70 GLM mHC Triton kernels (`sm70_mhc_prenorm_staging`, `sm70_mhc_post`, `sm70_mhc_pre_norm_from_staging`) that the dispatch above selects on Volta. The 1.3.0 wheel ships only the partial `hc_head_triton`. |
