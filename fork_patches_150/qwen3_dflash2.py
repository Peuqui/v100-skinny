...
def _use_sm70_bf16_emulation(config) -> bool:
    ...
    if not (is_bf16 and current_platform.is_cuda()):
        return False
...
class DFlash2Qwen3ForCausalLM(DFlashQwen3ForCausalLM):
    ...
    def compute_candidates(
        ...
        # A quantized target LM head is admissible here: the candidate TopK only
        # selects which tokens the selector drafts, and every draft is verified
        # against the target afterwards. Quantization can cost acceptance rate,
        # never correctness -- and the candidates then come from the very head
        # the target samples with. The regular branch below dispatches through
        # QuantizeMethodBase.apply, which every quantization method implements.
